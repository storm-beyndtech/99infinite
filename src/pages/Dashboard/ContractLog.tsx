...
import ContractSheet from "@/components/contract-sheet";
...
const ContractLog: React.FC = () => {
	...
	const fetchTransactions = async () => {
		try {
			setLoading(true);
			const response = await fetch(`${url}/transactions/contracts`);
	...
	useEffect(() => {
		...
		let filtered = txs.filter((transaction) => {
			// Filter by type (only contract transactions)
			if (transaction.type !== "contract") return false;
						...
						<div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8">
							...
							<div className="flex items-center gap-3">
								...
								>
									...
									<span className="max-sm:hidden">New</span> Contract
								</Link>
							</div>
						</div>
