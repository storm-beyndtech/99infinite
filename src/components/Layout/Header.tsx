      ...
      <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            {/* Slide-out Menu */}
            <div className={`fixed top-0 right-0 h-full w-80 bg-slate-900 shadow-2xl transform transition-transform duration-300 ease-in-out ${
              ...
              <div className="flex items-center justify-between p-6 border-b border-slate-700">
                ...
                <button
                  onClick={() => setIsMenuOpen(false)}
                  className="text-white hover:text-orange-400 p-2 rounded-lg hover:bg-white/10 transition-colors"
                >
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              <div className="px-6 py-6 space-y-1">
                {navigation.map((item) => (
                  <div key={item.name}>
                    ...
                    {item.hasDropdown && item.dropdownItems && mobileDropdown === item.name && (
                      <div className="pl-4 pb-2 space-y-1 bg-slate-800 rounded-lg mx-4 p-3 mt-2">
                        {item.dropdownItems.map((dropdownItem) => (
                          <Link
                            key={dropdownItem.name}
                            to={dropdownItem.href}
                ...
                <Link
                  to="/subscribe"
                  className="block mt-8 py-3 px-6 border-2 border-orange-400 text-center text-orange-400 hover:bg-orange-400 hover:text-cyan-900 font-semibold tracking-wide transition-colors rounded-lg"
                  onClick={() => setIsMenuOpen(false)}
                >
                  SUBSCRIBE
                </Link>
