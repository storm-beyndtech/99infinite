import type { Transaction } from "@/pages/Dashboard/AllTransactions";
import { TrendingUp, Shield, Users, Check, Clock, X, Calendar } from "lucide-react";

interface ContractSheetProps {
	contract: Transaction | null;
	isOpen: boolean;
	onClose: () => void;
}

const ContractSheet: React.FC<ContractSheetProps> = ({ contract, isOpen, onClose }) => {
	...
	const generateTransactionNumber = (transaction: Transaction) => {
		...
		return `CON-${year}${month}${day}-${shortId}`;
	};
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
									{generateTransactionNumber(contract)}
								</p>
							</div>
							<button
								onClick={onClose}
					...
					<div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
						...
						<div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 sm:p-6">
							<div className="flex items-center justify-between mb-6">
								<div className="flex items-center gap-3">
									{getPlanIcon(contract.planData?.plan ?? "")}
									<div>
										<h3 className="font-semibold text-slate-900 dark:text-slate-100">
											{contract.planData?.plan ?? "Unknown Plan"}
										</h3>
										<p className="text-sm text-slate-500 dark:text-slate-400">
											{contract.planData?.duration ?? "—"}
										</p>
									</div>
								</div>
								...
								<div>
									<div className="flex items-center justify-center gap-2 mb-1">
										<p className="text-2xl font-bold text-green-600 dark:text-green-400">
											{formatCurrency(contract.planData?.interest ?? 0)}
										</p>
									</div>
						...
						<div className="space-y-4">
							...
							<div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4">
								...
								<p className="text-sm text-slate-600 dark:text-slate-400">{formatDate(contract.date)}</p>
							</div>
						</div>
						...
						<div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4">
							...
							<div className="space-y-3">
								...
								<div className="flex justify-between items-center">
									...
									<span className="font-medium text-slate-900 dark:text-slate-100">
										{formatCurrency(contract.amount + (contract.planData?.interest ?? 0))}
									</span>
								</div>
								<div className="flex justify-between items-center">
									<span className="text-slate-600 dark:text-slate-400">Interest Rate</span>
									<span className="font-medium text-emerald-600 dark:text-emerald-400">
										{(((contract.planData?.interest ?? 0) / contract.amount) * 100).toFixed(2)} %
									</span>
								</div>
								<div className="flex justify-between items-center">
						...
						{contract.status === "completed" && (
							<div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
								...
								<p className="text-sm text-green-800 dark:text-green-200">
									Congratulations! Your contract has matured successfully. The total payout of{" "}
						...
						{contract.status === "cancelled" && (
							<div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 border border-red-200 dark:border-red-800">
								...
								<p className="text-sm text-red-800 dark:text-red-200">
									This contract was cancelled. Your principal amount of{" "}
									<span className="font-bold">{formatCurrency(contract.amount)}</span> has been refunded to
									your account.
								</p>
							</div>
						)}
						...
						{contract.status === "pending" && (
							<div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 border border-yellow-200 dark:border-yellow-800">
								...
								<p className="text-sm text-yellow-800 dark:text-yellow-200">
									Your contract is currently being processed. You will receive a confirmation once the
