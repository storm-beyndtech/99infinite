...
const formatCurrencyScaled = (value: number) => {
	const scaled = value * 1000; // treat data points as thousands
...
const getChartOptions = (): ApexOptions => ({
	...
	grid: {
		...
		row: { colors: undefined, opacity: 0.05 },
		column: { colors: undefined, opacity: 0.05 },
	...
	tooltip: {
		y: {
			formatter: function (val: number) {
				return formatCurrencyScaled(val);
			},
		},
	},
	legend: {
...
const ChartTwo: React.FC = () => {
	const [state] = useState<ChartTwoState>({
		series: [
			{ name: "Portfolio Value", data: [44, 55, 21, 27, 62, 43, 65] },
			{ name: "Contracts", data: [13, 23, 30, 48, 53, 27, 15] },
		],
	});
	...
	return (
		<div className="bg-white/40 dark:bg-gray-100/5 backdrop-blur-lg border border-gray-200/50 dark:border-white/20 rounded-xl p-6 shadow-xl h-full w-full">
			...
			<div>
				<div id="chartTwo" className="-mx-2"></div>
				<style>{`.apexcharts-text { fill: #6b7280 !important; } .apexcharts-legend-text { color: #6b7280 !important; }`}</style>
